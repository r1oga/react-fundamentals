...
- safe insert: .textContent, append
`before`, `prepend`, `append`, `after` are safe. They only work accept DOM nodes or text as inputs. Like `.textContent`

To insert HTML from a string (like `innerHTML`), use `insertAdjacentHTML` 
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=01%3A%20Basic%20JavaScript-rendered%20Hello%20World&em=r1oga%40tuta.io).
